...
- **`make build-all`**: 全てのターゲットプラットフォーム（Linux amd64/arm64、macOS arm64、Windows amd64）向けにクロスコンパイルを行い、`dist/` にバイナリを配置します。macOS は arm64 のみ（Apple Silicon）・Intel は非対応（Intel Mac はソースからビルド可能）。
- **`make package`**: 全てのバイナリをビルドし、各プラットフォーム向けのリリースアーカイブ（darwin/windows は `.zip`、linux は `.tar.gz`）を `dist/` ディレクトリに作成します。これらのアーカイブは配布準備ができています。
